# scenes/boss_event_k.py
print("【DEBUG】boss_event_k.py: import開始")
import pygame
...
bg_files = [
    ...
    "boss_k_bg1.png",
    "boss_k_bg1.png",
    "boss_k_bg2.png",  # ボスアップ
    # 必要に応じて追加
]

voice_files = [
    "boss_k_voice_00.mp3.enc",
    "boss_k_voice_01.mp3.enc",
    "boss_k_voice_02.mp3.enc",
    "boss_k_voice_03.mp3.enc",
    "boss_k_voice_04.mp3.enc",
    "boss_k_voice_05.mp3.enc",
    "boss_k_voice_06.mp3.enc",
    "boss_k_voice_07.mp3.enc",
]

script = [
    "もぐら：「あ、ごちみみずだ。もちもちみみずより希少なんだよね」",
    "ごちみみず：「そう！わたしはごちみみず！ついにこの日が来た！」",
    "もぐら：「肉厚で、身がぷりぷりで」",
    "ごちみみず：「おまえはもぐらくんだな！今日こそは同胞のかたきをとらさせてもらうぞ！」",
    "もぐら：「ほんのりとした甘みと濃厚なうまみが特徴なんだよね」",
    "ごちみみず：「わ、わたしの話を聞いていないのか…？」",
    "もぐら：「まさに、”ごちそう”を冠するにふさわしいみみずよね。」",
    "ごちみみず：「……！　もういい！　勝負だ、もぐらくん！」"
]
...
def handle_event(event, state, sound_manager=None):
    print("【DEBUG】boss_event_k.handle_event()呼び出し")
...
def draw(screen, font, WIDTH, HEIGHT, state, sound_manager=None):
    print("boss_event_k.py: draw関数入りました")
    print("【DEBUG】boss_event_k.draw()呼び出し")
