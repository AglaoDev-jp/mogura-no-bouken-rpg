...
# ◆ dialogに対応する音声ファイル名
voice_files = [
    "inn_voice_00.mp3",  # いらっしゃいませ、旅のお方。
    "inn_voice_01.mp3",  # 今晩、お休みになりますか？
]
...
def draw(screen, font, WIDTH, HEIGHT, state, sound_manager=None):
    ...
    # --- メッセージ表示（回復・失敗など） ---
        ...
        if (not state.get("voice_played", False)) and sound_manager is not None:
            voice_map = {
                "ゆっくり休めた！HPが全回復した": "inn_heal.mp3",
                "お金が足りないようですね…": "inn_no_money.mp3",
                "またのご利用をお待ちしております。": "inn_thankyou.mp3",
            }
