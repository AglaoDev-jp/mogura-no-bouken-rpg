...
# ◆ 会話開始メッセージ用の音声ファイル名
talk_voice = "shop_welcome.mp3"

# ◆ 商品ごとの購入音声
item_voice_map = {
    "ごちサンド": "shop_get_sand.mp3",
    "トロッコ": "shop_get_torocco.mp3",
    "スコップ": "shop_get_scoop.mp3",
    "ヘルメット": "shop_get_helmet.mp3",
}

# ◆ 結果メッセージに対する音声（退出など）
msg_voice_map = {
    "またきてね！": "shop_thankyou.mp3",
}
    ...
    if state["mode"] == "talk":
        ...
        if (not state.get("voice_played", False)) and sound_manager is not None:
            ...
            # --- すでに装備中（武器・防具共通音声） ---
                ...
                sound_manager.play_voice("shop_already_equip.mp3")
                state["voice_played"] = True
            # --- お金が足りないよ！ ---
            elif "お金が足りないよ！" in state["message"]: 
                sound_manager.play_voice("shop_no_money.mp3")
                state["voice_played"] = True
            # --- 退出（またきてね！） ---
