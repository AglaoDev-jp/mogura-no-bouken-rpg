    ...
    while running:
        ...
        for event in events:
            ...
            # オープニングイベント
                ...
                result = opening_handle_event(event, opening_state)
                if result == "end":
                    current_scene = "town"
                    break
            ...
            # === ボスイベント（ごちみみず） ===
                ...
                boss_event_k.draw(game_surface, font, WIDTH, HEIGHT, boss_event_k.state)
            ...
            # === ボスイベント（ふかふかだんご） ===
                ...
                boss_event_d.draw(game_surface, font, WIDTH, HEIGHT, boss_event_d.state)
                result = boss_event_d.handle_event(event, boss_event_d.state)
                if result == "battle":
                    sound_manager.play_bgm("Slapstick_Dance.mp3") 
                    from scenes import battle, dungeon
            ...
            # === ボスイベント（しゅごもぐら） ===
                ...
                # 次にキー入力をチェック
                result = boss_event_f.handle_event(event, boss_event_f.state)
                if result == "battle":
            ...
            # 戦闘シーン
                ...
                if result == "dungeon":
                    ...
                    # 逃走時の処理
                        ...
                        dungeon.current_floor = 0  # フロアを B1(0) にリセット
                        dungeon.player_pos = [1, 1]  # 階数と位置の初期化
                        current_castle_state = castle_init_state("gameover")
            ...
            # エンディングシナリオ
                ...
                result = ending.handle_event(event, ending.state)
                sound_manager.play_bgm("お昼のうた.mp3")
                
                if result == "staffroll":
                    staffroll.init()
                    current_scene = "staffroll"
                    break

            # エンドロール
            elif current_scene == "staffroll":
                result = staffroll.handle_event(event, staffroll.state)
                sound_manager.play_bgm("お昼のうた.mp3")
                if result == "title":
        ...
        try:
            ...
            if current_scene == "town":
                ...
                menu.draw(game_surface, font, WIDTH, HEIGHT)

            elif current_scene == "opening_event":
                opening_draw(game_surface, font, WIDTH, HEIGHT, opening_state)
            
            elif current_scene == "inn":
                print("  [DRAW] 宿屋描画")
                inn_draw(game_surface, font, WIDTH, HEIGHT, inn_state)
            elif current_scene == "shop":
                print("  [DRAW] 道具屋描画")
                shop.draw(game_surface, font, WIDTH, HEIGHT, shop_state)
            elif current_scene == "menu":
                print("  [DRAW] メニュー画面描画")
                menu.draw(game_surface, font, WIDTH, HEIGHT)
            elif current_scene == "inventory":
                ...
                print("  [DRAW] バトル描画")
                from scenes import battle
                battle.draw(game_surface, font, WIDTH, HEIGHT)

            elif current_scene == "castle":
                print("  [DRAW] 城描画")
                castle.draw(game_surface, font, WIDTH, HEIGHT, current_castle_state)

            elif current_scene == "ending":
                ending.draw(game_surface, font, WIDTH, HEIGHT, ending.state)

            elif current_scene == "staffroll":
                # 毎フレームスタッフロールの進行・描画
                staffroll.update(staffroll.state) # ←毎フレームスクロール！
                staffroll.draw(game_surface, font, WIDTH, HEIGHT, staffroll.state)

                # ここでフェードアウト処理
                if staffroll.state["finished"] and not staffroll.state.get("fading"):
                    sound_manager.fadeout_bgm(2000)  # 2秒でフェードアウト
                    staffroll.state["fading"] = True  # フェードアウト済みマーク
